...
window.SikatBGM = window.SikatBGM || {
    ...
    init() {
        ...
        this.audio.volume =
            parseFloat(localStorage.getItem("sikat_bgm_volume")) || 0.3;
        this.audio.preload = "auto";

        // Restore time position
        const savedTime =
            parseFloat(localStorage.getItem("sikat_bgm_time")) || 0;
        ...
        const wasPlaying =
            localStorage.getItem("sikat_bgm_state") === "playing";
        if (wasPlaying) {
            this.play();
        }
    },
    ...
    updateUI(isPlaying) {
        ...
        if (icon && indicator && btn) {
            if (isPlaying) {
                icon.className =
                    "fas fa-music text-white text-xl animate-pulse";
